...
ggplot(data = current_season ,aes(Date, Team.Average.RSI..m.per.s.)) +
  ...
  geom_hline(yintercept = team_mean - sqrt(team_var), color="#CFB87C") +
  labs(title="Team Average RSI through 2024-2025 Season",
       x="Date",
       y="RSI (m/s)")
...
ggplot(data = current_season, aes(x=Date, y=Change.Team.Average.RSI..m.per.s.)) +
  ...
  geom_hline(yintercept = 0,
             color = "#CFB87C",
             linewidth = 0.5) +
  labs(title = "Changes in Average Team RSI through 2024-2025 Season",
       x="Date",
       y="Change in RSI (m/s)") + 
  theme_classic()
...
colors <- c("Above Median Game" = "palegreen3", "Below Median Game" = "red3")
...
labs(title = "Changes in Average Team RSI with Game Outcomes through the 2024-2025 Season",
     x="Date",
     y="Change in RSI (m/s)")+
  theme_classic()
```
...
ggplot(data = current_season[current_season$anon_id == "ID_42",], aes(Date, Change.Player.Average.RSI..m.per.s.)) +
  geom_point() +
  geom_hline(yintercept=0, color = "#CFB87C") +
  labs(title="Changes in RSI for \"ID 42\" through the 2024-2025 Season",
       y="Change in RSI (m/s)") +
  theme_classic()
...
ggplot(data = current_season[current_season$anon_id == "ID_42",], aes(Date, Change.Player.Average.RSI..m.per.s.)) +
  ...
  geom_hline(yintercept=0) +
  geom_vline(xintercept = stats_clean$Date[stats_clean$anon_id == "ID_42" & (stats_clean$Player.Good.Game == 1 | stats_clean$Player.Bad.Game ==1)], color = ifelse(stats_clean[stats_clean$anon_id == "ID_42" & (stats_clean$Player.Good.Game == 1 | stats_clean$Player.Bad.Game ==1),]$Player.Good.Game == 1, "palegreen3","red3")) +
  labs(title="Changes in RSI for \"ID 42\" with Game Outcomes through the 2024-2025 Season",
       y="Change in RSI (m/s)") +
  theme_classic()
```
...
# Create a plot for each athlete that shows the relationship between RSI and average acceleration load
# 7-Day
...
ggplot(data=VALD_ForceDecks_clean, aes(x=Date, y=Player.Average.RSI..m.per.s.)) +
  geom_point() +
  geom_smooth(method="lm", color="#CFB87C", se=FALSE) +
  labs(title="RSI Measurements Throughout 2024-2025 Season",
       x="Date",
       y="RSI (m/s)") +
  theme_classic()
...
p <- ggplot(data=VALD_ForceDecks_clean[VALD_ForceDecks_clean$anon_id == IDs[i],], aes(x=Date, Player.Average.RSI..m.per.s.)) +
  ...
  labs(title = "Significant Player RSI Measurements for 2024-2025 Season", subtitle = IDs[i],
       y="RSI (m/s)") +
  theme_minimal()
...
ggplot(data=test, aes(x=Date)) +
  ...
  labs(title = "Number of Players Above Their Mean Season RSI Measurement",
       y="Count Above Mean RSI Measurement") +
  theme_classic()
...
ggplot(data=test, aes(x=Date)) +
  ...
  labs(title = "Number of Players Below Their Mean Season RSI Measurement",
       y="Count Below Mean RSI Measurement") +
  theme_classic()
```
...
```{r Looking at means and SDs in RSI for each player}
#group by and summarize to see each player's mean and sd in RSI
VALD_ForceDecks_clean %>%
  group_by(anon_id) %>%
